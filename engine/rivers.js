...
//
// Зависимости: mulberry32, getHeight, PerlinNoise (косвенно через heightmap).
//
// В Node.js: подтягиваем mulberry32/getHeight из engine/noise.js через require().
// В браузере: берём из window.
// ══════════════════════════════════════════════════════════════════════
...
(function(root) {
  ...
  /**
   * Мини-fallback для getHeight, если модуль noise.js не загружен.
   * (используется только как защита — в реальной жизни noise грузится первым)
   */
  ...
  var MOUNTAIN_H  = 0.78;  // старт рек — горные зоны
  var WATER_H     = 0.20;  // конец рек — дошли до воды
   ...
   */
  function traceRiver(heightmap, startX, startY, maxSteps) {
   ...
   */
  function generateRivers(heightmap, count, seed) {
    if (!heightmap || !heightmap.data) return [];
    if (!(count > 0)) return [];
  ...
  if (root) {
    root.traceRiver     = traceRiver;
    root.generateRivers = generateRivers;
  }
