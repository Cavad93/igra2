...
| 31–43 | Рефакторинг index.html (Часть II) | Вынос CSS и JS в `ui/styles/` и `ui/*.js` |
| 44–54 | Рефакторинг engine/turn.js (Часть III-A) | Разбиение монолита на 9 модулей |
| 55–66 | ES-модули (Часть III-B) | Убрать onclick, window.*, перейти на import/export |
| 67–71 | Vite (Часть III-C) | Build-step: dev-сервер, prod-сборка, code-splitting |
| 72 | Финальное расщепление CSS | `ui/styles/map.css` |
| 73 | Финальное расщепление CSS | `ui/styles/diplomacy.css` |
| 74 | Финальное расщепление CSS | `ui/styles/regions.css` |
| 75 | Финальное расщепление CSS | `ui/styles/population.css` + `economy.css` |
| 76 | Финальное расщепление CSS | `ui/styles/characters.css` |
| 77 | Финальное расщепление CSS | `ui/styles/government.css` |
| 78 | Финальное расщепление CSS | `ui/styles/overlays.css` + `battle.css` |
| 79 | Финальное расщепление CSS | Остатки в `base.css` / `responsive.css` |
| 80 | Финальное расщепление CSS | Аудит: `index.html` < 2000 строк |
...
## ЭТАП 61 — Модули: конвертировать `data/*.js` в ES-модули ✅ ВЫПОЛНЕНО
...
## ЭТАП 62 — Модули: конвертировать `engine/*.js` в ES-модули ✅ ВЫПОЛНЕНО
...
## ЭТАП 63 — Модули: конвертировать `ui/*.js` в ES-модули ✅ ВЫПОЛНЕНО
...
## ЭТАП 64 — Модули: конвертировать `ai/*.js` в ES-модули ✅ ВЫПОЛНЕНО
...
## ЭТАП 65 — Модули: единый `<script type="module" src="ui/boot.js">` ✅ ВЫПОЛНЕНО
...
## ЭТАП 66 — Модули: финальный аудит, 0 глобалов, 0 inline onclick ✅ ВЫПОЛНЕНО
...
## ЭТАП 67 — Vite: инициализация проекта ✅ ВЫПОЛНЕНО
...
## ЭТАП 68 — Vite: настройка dev-сервера с HMR ✅ ВЫПОЛНЕНО
...
## ЭТАП 69 — Vite: продакшн-сборка ✅ ВЫПОЛНЕНО
...
## ЭТАП 71 — Финальный аудит Части III ✅ ВЫПОЛНЕНО
...
## ЧАСТЬ IV — Финальное расщепление `index.html`

**Цель:** довести `index.html` до < 2000 строк, как обещано в метриках этапа 71.

**Контекст аудита (на момент начала Части IV):**

- `index.html` = 8688 строк.
- Из них inline `<style>` занимает **7779 строк** (строки 37–7815, ≈ 90% файла,
  2117 CSS-правил в ~30 логических секциях).
- Ранее вынесено 5 файлов в `ui/styles/` (base, top, splash, panels, bottom),
  суммарно 3197 строк. Этого оказалось недостаточно.
- HTML-разметка `<body>` = 873 строки (строки 7816–8688), её не трогаем —
  это реальный каркас интерфейса.

**Принцип:** каждый этап выносит одну логическую группу CSS в отдельный
файл `ui/styles/*.css` и подключает его через `<link>` в том же порядке
каскада, что и сейчас. Разметка HTML остаётся на месте. После каждого
этапа — smoke-тест и коммит.

**Правила для всех этапов 72–80:**

1. Перед удалением из `index.html` — **сначала** скопировать правила в
   новый CSS-файл и подключить его через `<link>`. Только после того как
   визуальная регрессия проверена — удалить инлайн-правила и заменить
   комментарием-маркером.
2. Порядок `<link>`'ов в `<head>` должен сохранять каскад: base → top →
   splash → panels → bottom → новые файлы → (инлайн-остатки). Новые
   `<link>`'и ставить **после** `bottom.css` но **до** закрывающего `</style>`,
   чтобы инлайн-правила из `<style>` могли переопределять (если такая
   зависимость выявится).
3. Media queries (`@media`) переносить **вместе** с правилами которые они
   оборачивают. Не разделять блок `@media { … }` между файлами.
4. `@keyframes` анимации переносить в тот же файл где используются. Если
   анимация используется в нескольких секциях — оставить в `base.css`
   (общий слой).
5. Smoke-тест после каждого этапа: `npm run dev`, открыть игру, протестировать
   именно ту фичу чей CSS был вынесен.

---

## ЭТАП 72 — Рефакторинг: вынос CSS карты в `ui/styles/map.css` ✅ ВЫПОЛНЕНО
...
Самая большая и самая связная секция оставшегося inline-CSS — стили карты.
Сюда входят: Leaflet-overrides, уровни зума (strategic/regional/detailed),
режимы карты (`#map-mode-bar`), роза ветров, армии-орлы, подписи столиц,
маршруты, пульсы событий, AI-индикаторы действий.

Суммарно ≈ **2150 строк**, разбитых на два больших непрерывных блока.

---

### Что читать перед началом

1. Открыть `index.html`. Найти в инлайн-`<style>` два блока:
   - **Блок A** (строки ≈ 378–1074): `ЦЕНТРАЛЬНАЯ ОБЛАСТЬ — КАРТА`,
     Leaflet-overrides, zoom-levels, map-breathe.
   - **Блок B** (строки ≈ 2386–3832): map modes, роза ветров, армии,
     города, маршруты, пульсы, event feed, AI-indicators, siege, army route preview.
2. Прочитать каждый блок целиком, убедиться что его границы соответствуют
   карточно-связанным селекторам (`.leaflet-*`, `#map-*`, `.army-*`,
   `.city-*`, `.wind-rose*`, `.event-feed*`).
3. Проверить `ui/map.js` на наличие JS который дописывает классы — чтобы
   при переносе не потерять ни одного селектора.

---

### Что сделать

**Шаг 72.1 — Создать `ui/styles/map.css`**

```css
/* ───────────────────────────────────────────────────────────
   map.css — стили карты и всех визуальных элементов на ней:
   Leaflet, zoom-levels, map-modes, wind-rose, armies,
   cities, routes, event-feed, AI-indicators.
   Рефакторинг Части IV, этап 72 (uisuper.md).
   Вынесено из index.html <style> блоков 378–1074 и 2386–3832.
   ─────────────────────────────────────────────────────────── */
```

**Шаг 72.2 — Скопировать правила из блоков A и B**

Копировать **дословно**, сохраняя порядок. Включая комментарии-разделители
и `@keyframes map-breathe` если он есть в этих блоках.

**Шаг 72.3 — Подключить `<link>` в `<head>`**

После `bottom.css`:
```html
<link rel="stylesheet" href="ui/styles/map.css">
```

**Шаг 72.4 — Удалить перенесённые правила из `<style>`**

Заменить удалённые блоки комментариями:
```css
/* map.css: Leaflet, zoom, map-modes, wind-rose, armies, routes,
   event-feed, AI-indicators вынесены в ui/styles/map.css (этап 72). */
```
...
2. Карта отображается корректно: тайлы, регионы, дороги.
3. Zoom in/out работает — классы `map-zoom-strategic|regional|detailed`
   переключаются, стили применяются.
4. Армии-орлы видны на карте, корректно позиционированы.
5. Подписи столиц в `detailed` уровне появляются.
6. Роза ветров отображается, переключает режимы.
7. `#map-mode-bar` переключает режимы карты.
8. `wc -l index.html` уменьшилось на ≈ 2150 строк.
9. DevTools Console — 0 новых ошибок.
...
```
refactor(ui): этап 72 — вынос CSS карты в ui/styles/map.css
```

---

*Следующий этап: вынос CSS дипломатии в `ui/styles/diplomacy.css`.*

---


## ЭТАП 73 — Рефакторинг: вынос CSS дипломатии в `ui/styles/diplomacy.css` ✅ ВЫПОЛНЕНО
...
Дипломатия — вторая по объёму группа стилей (≈ 2000 строк). Распределена
по нескольким несвязанным блокам в инлайн-`<style>`:

- Базовые стили диплоэкрана (≈ 1647–1783)
- Вкладка переговоров `diplomacy_tab.js` (≈ 6055–6359)
- Большое диплоокно (≈ 6360–7165)
- Чат-мессенджер (≈ 7166–7473)
- Финализация договора (≈ 7474–7711)

Этап 70 запланировал `diplomacy_tab.js` как lazy-load; чтобы ленивая
загрузка работала эффективно, её CSS тоже должен быть отдельным файлом.

---

### Что читать перед началом

1. Найти в инлайн-`<style>` все блоки с селекторами `.diplo-*`,
   `#diplomacy-*`, `#treaty-*`, `#messenger-*`, `#negotiation-*`.
2. Сверить с `ui/diplomacy_tab.js` — какие классы он создаёт динамически.
3. Проверить нет ли пересечений с `ui/styles/panels.css` (некоторые
   модалки могли уже быть вынесены).

---

### Что сделать

**Шаг 73.1 — Создать `ui/styles/diplomacy.css`** с шапкой-комментарием.

**Шаг 73.2 — Скопировать все 5 диплоблоков в `diplomacy.css`** в том же
порядке (базовые → вкладка → большое окно → мессенджер → финализация),
чтобы каскад переопределений сохранился.

**Шаг 73.3 — Подключить `<link>` после `map.css`:**
```html
<link rel="stylesheet" href="ui/styles/diplomacy.css">
```

**Шаг 73.4 — Удалить перенесённые блоки, заменив маркерами.**

---

### Проверка перед коммитом

1. Открыть игру, начать ход, открыть дипломатию.
2. Список наций отображается, клик по нации открывает переговоры.
3. Мессенджер работает: ввод → отправка → ответ AI.
4. Договор: финализация → кнопки «Принять/Отклонить».
5. Большое диплоокно: карточки наций, фильтры отношений.
6. `wc -l index.html` уменьшилось на ≈ 2000 строк.
...
```
refactor(ui): этап 73 — вынос CSS дипломатии в ui/styles/diplomacy.css
```

---

*Следующий этап: вынос CSS панели региона (культура, религия, вкладки, стройка).*

---

## ЭТАП 74 — Рефакторинг: вынос CSS региона в `ui/styles/regions.css` ✅ ВЫПОЛНЕНО
...
Стили карточек и оверлеев, связанных с регионом и его экраном:
- Блок религии в карточке региона (≈ 1075–1171)
- Окно культуры — Modern Antiquity redesign (≈ 1172–1500)
- Окно религии — дополнительные стили (≈ 1501–1646)
- Вкладки панели региона (≈ 5366–5780)
- Аккордеон строительства — вариант B (≈ 5781–6020)
...
1. Найти в инлайн-`<style>` все 5 блоков по комментариям-разделителям.
2. Сверить с `ui/panels.js` и `ui/construction_accordion.js` (если есть)
...
**Шаг 74.1 — Создать `ui/styles/regions.css`** с шапкой.

**Шаг 74.2 — Скопировать 5 блоков в том же порядке.**

**Шаг 74.3 — Подключить `<link>` после `diplomacy.css`:**
```html
<link rel="stylesheet" href="ui/styles/regions.css">
```

**Шаг 74.4 — Удалить перенесённые блоки, заменив маркерами.**

---

### Проверка перед коммитом

1. Клик по региону → открывается панель с вкладками.
2. Вкладка «Культура» рендерится, кнопки и слайдеры работают.
3. Вкладка «Религия» рендерится, переключение культов работает.
4. Аккордеон «Строительство» раскрывается, кнопки постройки активны.
5. `wc -l index.html` уменьшилось на ≈ 1225 строк.
...
```
refactor(ui): этап 74 — вынос CSS региона в ui/styles/regions.css
```

---

*Следующий этап: оверлеи населения и экономики.*
...
Два крупных оверлея со своими стилями:
- **Население (Dashboard + возрастная демография)** ≈ 690 строк
  (≈ 4462–5149 инлайн)
- **Экономический обзор React + анимации** ≈ 250 строк
  (≈ 5150–5365 и 6021–6054 инлайн)

Оба оверлея — lazy-load кандидаты этапа 70. Раздельные CSS-файлы
позволят вынести и стили в lazy-chunk в будущем.

---

### Что читать перед началом

1. Найти в инлайн-`<style>`:
   - блок `ОВЕРЛЕЙ НАСЕЛЕНИЯ v2 — DASHBOARD` и `ВОЗРАСТНАЯ ДЕМОГРАФИЯ`.
   - блок `ЭКОНОМИЧЕСКИЙ ОБЗОР — React overlay` и
     `ЭКОНОМИЧЕСКИЙ ОБЗОР (React) — анимации и глобальные стили`.
2. Сверить с `ui/population_tab.js` и `ui/economy_react.jsx` (или
   `ui/economy_tab.js`).
...
**Шаг 75.1 — Создать `ui/styles/population.css`** с шапкой. Перенести
оба блока населения.

**Шаг 75.2 — Создать `ui/styles/economy.css`** с шапкой. Перенести
оба блока экономики (основной и анимации) в исходном порядке.

**Шаг 75.3 — Подключить оба `<link>`** после `regions.css`:
```html
<link rel="stylesheet" href="ui/styles/population.css">
<link rel="stylesheet" href="ui/styles/economy.css">
```

**Шаг 75.4 — Удалить перенесённые блоки, оставив маркеры.**

---

### Проверка перед коммитом

1. Открыть оверлей населения: демография, пирамида возрастов,
   карточки городов — всё рендерится.
2. Открыть экономический обзор: графики, балансы, анимации появления.
3. Закрытие оверлеев работает.
4. `wc -l index.html` уменьшилось на ≈ 940 строк.
...
```
refactor(ui): этап 75 — вынос CSS населения и экономики
```

---

*Следующий этап: персонажи и портреты.*

---

## ЭТАП 76 — Рефакторинг: вынос CSS персонажей в `ui/styles/characters.css` ✅ ВЫПОЛНЕНО
...
Стили всего что связано с людьми (советники, правитель, претенденты,
заговорщики):
- Раздел `ПЕРСОНАЖИ` (≈ 2073–2105)
- `ШАГ 57 — CC0-портреты персонажей` (≈ 2106–2189)
- `ШАГ 43 — ЭКРАН ДОЛЖНОСТЕЙ (ДВОР)` (≈ 2190–2320)
- `Этапы 23/24 — COURT BOARD (EU4/Imperator)` (≈ 2321–2355)
- `TOOLTIP ДЛЯ СИМВОЛОВ НАЦИИ` (≈ 2356–2385)
- `ИНИЦИАТИВЫ ПЕРСОНАЖЕЙ` (≈ 4308–4336)
- `ЗАГОВОРЩИКИ ИГРОКА` (≈ 4337–4356)
...
**Шаг 76.1 — Создать `ui/styles/characters.css`** с шапкой.

**Шаг 76.2 — Перенести все 7 блоков** в исходном порядке появления.

**Шаг 76.3 — Подключить `<link>`** после `economy.css`.

**Шаг 76.4 — Заменить маркерами в инлайн-`<style>`.**

---

### Проверка перед коммитом

1. Камеи-советники на правой панели рендерятся.
2. Court Board (двор) открывается, должности назначаются.
3. Портреты (CC0) подгружаются.
4. Tooltip символа нации работает при hover.
5. Инициативы персонажей отображаются; заговорщики видны.
6. `wc -l index.html` уменьшилось на ≈ 365 строк.
...
```
refactor(ui): этап 76 — вынос CSS персонажей в ui/styles/characters.css
```

---

*Следующий этап: правительство, сенат, законы, дебаты.*

---

## ЭТАП 77 — Рефакторинг: вынос CSS правительства в `ui/styles/government.css` ✅ ВЫПОЛНЕНО
...
Сенат, законы и дебаты — отдельный модуль с богатой визуалкой:
- `ОВЕРЛЕЙ ПРАВИТЕЛЬСТВА` (≈ 2599–2658)
- `ФОРМА СОСТАВЛЕНИЯ ЗАКОНА` (≈ 3833–3882)
- `ЗАЛ ДЕБАТОВ — анимированное голосование` (≈ 3883–4176)
...
**Шаг 77.1 — Создать `ui/styles/government.css`** с шапкой.

**Шаг 77.2 — Перенести три блока в исходном порядке.**

**Шаг 77.3 — Подключить `<link>` после `characters.css`.**

**Шаг 77.4 — Заменить маркерами.**

---

### Проверка перед коммитом

1. Открытие оверлея правительства: фракции, должности, модификаторы.
2. Создание закона: форма отображается, поля валидируются.
3. Зал дебатов: анимация голосования, табло результатов, итог.
4. `wc -l index.html` уменьшилось на ≈ 405 строк.
...
```
refactor(ui): этап 77 — вынос CSS правительства в ui/styles/government.css
```

---

*Следующий этап: оверлеи событий + батальные стили.*
...
Две логические группы в одном этапе — каждая достаточно небольшая, но
связана с оверлеями событий и боевыми экранами:

**overlays.css** (≈ 285 строк):
- `ЛОГ — ФИЛЬТРЫ` (≈ 4177–4189)
- `ВЫБОР СОБЫТИЯ` (≈ 4190–4211)
- `ИТОГИ ХОДА` (≈ 4212–4239)
- `ПАНЕЛЬ МИРНЫХ ПЕРЕГОВОРОВ` (≈ 4240–4284)
- `КОНЕЦ ИГРЫ` (≈ 4285–4307)
- `МОДАЛ API КЛЮЧА` (≈ 4357–4461)

**battle.css** (≈ 1020 строк):
- `РЕЗУЛЬТАТ БИТВЫ (#battle-result-overlay)` (≈ 2921–3117)
- `ПАНЕЛЬ ОСАДЫ (#siege-panel)` (≈ 3118–3126)
- `Шаг 47 — Предпросмотр маршрута армии (hover)` (≈ 3127–3832)
- `ТАКТИЧЕСКИЙ БОЙ — D1: CSS-переменные` (≈ 7712–7815)
...
**Шаг 78.1 — Создать `ui/styles/overlays.css` и `ui/styles/battle.css`**
с шапками-комментариями.

**Шаг 78.2 — Перенести блоки** в соответствующие файлы в исходном порядке.

**Шаг 78.3 — Подключить оба `<link>`** после `government.css`:
```html
<link rel="stylesheet" href="ui/styles/overlays.css">
<link rel="stylesheet" href="ui/styles/battle.css">
```

**Шаг 78.4 — Заменить маркерами в инлайн-`<style>`.**

---

### Проверка перед коммитом

1. Выбор события: оверлей открывается, кнопки выбора работают.
2. Итоги хода: модалка с балансами и дельтами.
3. Конец игры: модалка финала.
4. Модал ввода API-ключа: форма, валидация.
5. Битва: результат-оверлей, список потерь.
6. Осада: панель осады и хода.
7. Предпросмотр маршрута армии при hover — корректный рендер.
8. Тактический бой: базовые CSS-переменные применяются.
9. `wc -l index.html` уменьшилось на ≈ 1305 строк.
...
```
refactor(ui): этап 78 — вынос CSS оверлеев и боя
```

---

*Следующий этап: финальные остатки в `base.css` + `responsive.css`.*

---

## ЭТАП 79 — Рефакторинг: остатки в `base.css` / `responsive.css` ✅ ВЫПОЛНЕНО
...
После этапов 72–78 в инлайн-`<style>` останутся только мелкие общие блоки:

- Иконки, layout, статус-бар, toast, pulse (≈ 37–378, где что-то не
  попало в base/top/panels ранее) — ≈ 340 строк
- `АДАПТИВНОСТЬ` — media queries (≈ 1784–1809)
- `ПАНЕЛИ — ОБЩИЕ СТИЛИ` — если остались хвосты (≈ 1819–2072) — ≈ 260 строк
...
**Шаг 79.1 — Общие правила layout, toast, pulse, иконок → дополнить
`ui/styles/base.css`** (не создавать новый файл, добавить секцию в
существующий base.css после font-roles).

**Шаг 79.2 — Если блок `ПАНЕЛИ — ОБЩИЕ СТИЛИ` содержит правила, которые
не попали в panels.css — перенести остаток в panels.css.**

**Шаг 79.3 — Создать `ui/styles/responsive.css`** и перенести туда все
`@media`-блоки из секции АДАПТИВНОСТЬ. Подключить последним:
```html
<link rel="stylesheet" href="ui/styles/responsive.css">
```

**Шаг 79.4 — Удалить перенесённые правила из инлайн-`<style>`.**

---

### Проверка перед коммитом

1. Все базовые UI-элементы (layout, иконки, toast, pulse) работают.
2. Ресайз окна браузера применяет media queries (проверить на
   типовых breakpoints).
3. `wc -l index.html` уменьшилось на ≈ 630 строк.
...
```
refactor(ui): этап 79 — остатки inline-CSS в base.css / responsive.css
```

---

*Следующий этап: финальная чистка и аудит.*

---

## ЭТАП 80 — Финальная чистка и аудит Части IV ✅ ВЫПОЛНЕНО
...
После этапов 72–79 в инлайн-`<style>` должно остаться 0–50 строк
(критические above-fold правила или совсем мелкие остатки).

---

### Что сделать

**Шаг 80.1 — Аудит инлайн-`<style>`**

Пробежаться по всем оставшимся правилам. Для каждого:
- если правило вообще нужно (селектор ещё используется в HTML/JS) →
  перенести в соответствующий `*.css` файл по смыслу.
- если селектор больше не используется → удалить.

**Шаг 80.2 — Если `<style>` пуст — удалить тег полностью.**

Допустимо оставить `<style>` только если там < 20 строк критического
above-fold CSS (например стиль сплэша, который должен примениться до
загрузки любых `<link>`).

**Шаг 80.3 — Проверить порядок `<link>`'ов**
...
```bash
wc -l index.html                                 # ожидание: < 2000
grep -c '<link rel="stylesheet"' index.html     # ожидание: ≈ 15
wc -l ui/styles/*.css                            # сумма ≈ 10 000–11 000
```

**Шаг 80.5 — Полный smoke-тест в dev и preview**

Пройти чеклист из этапа 71 шаг 71.2, оба режима (`npm run dev` и
`npm run preview`) — ни одной визуальной регрессии.

**Шаг 80.6 — Обновить `docs/refactor_modules.md`**

Добавить секцию «Часть IV — финальное расщепление CSS»:
- начальные метрики (index.html = 8688, inline style = 7779)
- финальные метрики (index.html < 2000, 0 inline CSS)
- список всех 15 CSS-файлов с размерами.

---

### Проверка перед коммитом

1. `wc -l index.html` < 2000.
2. Инлайн-`<style>` либо отсутствует, либо содержит < 20 строк.
3. Все 15 CSS-файлов подключены в правильном порядке.
4. Полный smoke-тест: 0 визуальных регрессий, 0 ошибок в консоли.
5. Bundle в `npm run build` собирается без ошибок.
6. Все этапы 72–80 помечены как ✅ ВЫПОЛНЕНО в этом файле.
...
```
refactor(ui): этап 80 — финальная чистка inline-CSS, index.html < 2000 строк
```
...
| Метрика | До Части IV | После Части IV |
|---------|-------------|-----------------|
| `index.html` строк | 8 688 | < 2 000 |
| Inline `<style>` строк | 7 779 | 0–20 |
| Файлов в `ui/styles/` | 5 | 15 |
| Суммарно CSS-строк | 3 197 | ~11 000 |
| CSS-файлов в lazy-chunk кандидатах | 0 | 3 (diplomacy, population, economy) |

Теперь `index.html` — тонкий каркас (`<head>` + разметка + один
`<script type="module">`), без inline-стилей. Каждая визуальная
группа изолирована в своём CSS-файле, что упрощает HMR, code-review,
code-splitting и будущие редизайны.
