...
## MIL_009 — Перебалансировка атриции и усталости (2026-03-31)

### Сделано:
- `SUPPLY_ATTRITION_THRESHOLD: 30 → 50` — атриция начинается раньше ✓
- `ATTRITION_RATE: 0.02 → 0.025` — усиленные потери от голода ✓
- `FATIGUE_MARCH: 12 → 14` — марш утомляет сильнее ✓
- `FATIGUE_REST_FRIENDLY: -15 → -10` — восстановление медленнее ✓
- `FATIGUE_REST_ENEMY: -5 → -3` — отдых на вражеской ещё труднее ✓
- Наёмники теряют вдвое меньше (`mercFactor = 0.5`) при атриции ✓
- Штраф морали `-2` при `supply < 30` с реальными потерями ✓
- Лог-событие `"[Армия] несёт потери от истощения ([N] бойцов)"` при `rate > 0.015` ✓
- Формула: `rate = ATTRITION_RATE * (1 - supply / THRESHOLD)` (использует именованный порог) ✓
- Тесты: 27/27 ✅ (tests/mil_009_attrition_test.cjs)
- Регрессии: MIL_002 16/16, MIL_003 18/18, MIL_004 16/16, MIL_005 23/23, MIL_006 18/18, MIL_007 16/16, MIL_008 15/15 ✅
