...
async function getAIBatchDecisions(nationIds, model = CONFIG.MODEL_HAIKU) {
  ...
  const nationBlocks = nationIds.map(id => {
    ...
    // Последние OU Fallback решения — чтобы Groq знал что происходило между ротациями
  ...
  войны:[${wars.join(',')}]
  действия:[${actions.join(',')}]${recentOU ? `\n  последние_решения:${recentOU}` : ''}`;
  }).filter(Boolean).join('\n\n');

  const system = `Ты стратег-советник. Прими решение для каждой нации.
Учитывай последние_решения нации — они показывают что делал автопилот между твоими ходами.
...
target: для attack/trade/diplomacy — id нации или региона, иначе null.`;
  ...
  // Масштабируем токены ответа пропорционально числу наций
  const responseTokens = Math.min(2000, Math.max(600, nationIds.length * 80));
