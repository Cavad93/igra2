...
> **Статус:** ✅ **ВЫПОЛНЕНО** — `assets/manifest.json` содержит 10 записей с `"group":"backgrounds"` (по одной на культурную группу): `splash_pompeii` (Met 247008 — Boscoreale Villa wall painting, 2.74 MB), `splash_alexander` (CMA 1987.65 — Mosaic of Tigress and Cubs, 782 KB), `splash_carthage` (CMA 2008.349 — «Hannibal Swearing Eternal Enmity toward Rome», 799 KB), `splash_nome_gods` (CMA 1961.205 — Nome Gods Bearing Offerings, 865 KB), `splash_persepolis` (CMA 1943.279 — Fragment from Palace of Xerxes, 501 KB), `splash_celtic_head` (CMA 1955.555 — Celtic Head, 378 KB), `splash_gandhara_stupa` (CMA 1930.328 — Gandhara stupa relief, 321 KB), `splash_night_white` (Met 39901 — Night-Shining White, 2.60 MB), `splash_scythian_stag` (CMA 1985.77 — Stag Plaque, 547 KB), `splash_battle` (CMA 1929.979 — Black-Figure Neck-Amphora с батальной сценой, 343 KB). Все источники — Met/CMA CC0 1.0 (без Wikimedia, чтобы пройти `bash assets/download.sh` в любой sandbox-среде). `data/culture_groups.js` обновлён — все 10 `splash_bg` полей уникальны (greek→pompeii, roman→alexander, carthaginian→carthage, egyptian→nome_gods, persian→persepolis, celtic→celtic_head, indian→gandhara_stupa, east_asian→night_white, nomadic→scythian_stag, generic→battle). `ui/splash.js` резолвит bg через `getCultureGroup` + `--splash-bg` CSS-переменную. CSS фильтр `sepia(0.25) brightness(0.55)` в `.splash__bg`. Тесты: `tests/test_arma_stage67.mjs` (123 passed), также `tests/test_arma_stage58.mjs` обновлён (50 passed). Не реализовывать повторно.
...
> **Статус:** ✅ **ВЫПОЛНЕНО** — 10 SVG-иконок культурных групп и 2 варианта декоративной рамки лежат в `assets/icons/` и в git. Спецификация Шага 68 требует имена `owl_athena`, `roman_eagle`, `carthage_star`, `egyptian_ankh`, `persian_faravahar`, `celtic_torque`, `indian_lotus`, `east_asian_dragon`, `nomadic_bow`, `generic_sword`, `border_meander_gold`, `border_meander_dark` — все 12 файлов присутствуют. Существующие иконки `carthage_horse.svg` / `egyptian_eye.svg` / `meander_gold.svg` / `meander_dark.svg` (из Шагов 59/60) сохранены: `data/culture_groups.js` ссылается на них, замена сломала бы `applyNationTheme`/`updateNationHeader`/`getNationIconPath`. В рамках Шага 68 добавлены 4 файла-эквивалента под именами из спецификации: `assets/icons/carthage_star.svg` (восьмилучевая звезда + знак Танит), `assets/icons/egyptian_ankh.svg` (анкх с петлёй и перекладиной), `assets/icons/border_meander_gold.svg` и `assets/icons/border_meander_dark.svg` (греческий меандр-уголок 60×60). Все SVG: `viewBox="0 0 32 32"` для иконок и `viewBox="0 0 60 60"` для рамок, без `<linearGradient>`/`<radialGradient>`/`<filter>`, размер ≤ 2 KB. Тесты `tests/test_arma_stage68.mjs` (102 passed): проверяют git ls-files, наличие на диске, валидность через `xmllint`, размер ≤ 2 KB, отсутствие градиентов/фильтров, успешное завершение `bash assets/download.sh`, `ls assets/portraits/greek/ ≥ 4` и валидность `manifest.json`. Не реализовывать повторно.
...
> **Статус:** ✅ **ВЫПОЛНЕНО** — в `assets/manifest.json` добавлены 18 новых CC0-записей (6 в `indian/` + 7 в `greek/` + 5 в `roman/`). Источник — The Cleveland Museum of Art (CC0 1.0), поскольку Met Museum public-search по `gandhara+head`/`gandhara+bodhisattva` в 2026 возвращает `{total:0}` (полнотекстовый индекс сломан по данному запросу), а CMA Open Access API стабильно отдаёт релевантную Gandhāra/Kushan-выборку (`/api/artworks?q=kushan&cc0=1&type=Sculpture` → 83 скульптуры). Новые Gandhāra/Indian записи: `indian/cma115946` (1962.47 Head of Jina), `indian/cma154769` (1989.346 Plaque with a Male Head, Gandhara), `indian/cma147820` (1971.41 Head of a Rishi), `indian/cma155376` (1986.71 Male Fertility Divinity / Shiva), `indian/cma143679` (1967.40 Layman Bearing Offerings, Gandhara schist), `indian/cma143680` (1967.41 Monk Bearing Offerings, Gandhara schist). Новые мраморные греческие головы (1928.858 Sphinx, 1924.538 Kore, 1938.426 Woman, 1929.440 Foreign Warrior, 1929.980 Woman with Sistrum, 1971.71 Herakles, 1924.876 Man). Новые римские бюсты (1925.944 Man, 1925.945 Balbinus, 1925.468 Trajan, 1925.161 Faustina the Younger, 1952.260 Noble/Official). После `bash assets/download.sh`: `ls assets/portraits/indian/` = **16** ≥ 15 ✓, `ls assets/portraits/greek/` = **25** ≥ 25 ✓, `ls assets/portraits/roman/` = **22** ≥ 20 ✓. Все JPG валидны (magic `FF D8 FF`). Тесты `tests/test_arma_stage70.mjs` (64 passed). Не реализовывать повторно.
...
> **Статус:** ✅ **ВЫПОЛНЕНО** — `assets/manifest.json` расширен с 106 до 159 записей (+53 новых CC0-портрета). Все 10 культурных папок (`greek`, `roman`, `egyptian`, `persian`, `celtic`, `indian`, `east_asian`, `nomadic`, `carthaginian`, `generic`) содержат ≥ 6 записей. Источники: Met Museum (4 египетских объекта: 244464 бронзовый фараон, 549358 плакетка царя, 543869 коленопреклонённый пленник, 544864 Таурт) + Cleveland Museum of Art (49 объектов — египетские цари/царицы, ахеменидские/сасанидские изделия, пунические монеты и финикийские пластины, кельтские фибулы и монеты, скифские/ордосские плакетки, танские придворные фигуры). Wikimedia Commons недоступна из песочницы (блокировка egress-policy) — заменено эквивалентными CC0-источниками Met/CMA, как допускается `assets/download.sh` (те же лицензии, тот же уровень культурной точности). Тесты: `tests/test_arma_stage71.mjs` (68 passed). Не реализовывать повторно.
...
### Шаг 72 — CSS-вариации: умножить пул портретов в 8 раз без новых файлов ✅ ВЫПОЛНЕНО
